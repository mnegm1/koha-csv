...
const COMMON_GIVEN = new Set(["محمد","احمد","أحمد","علي","حسن","حسين","خالد","سعيد","سالم","يوسف","عبدالله","عبد الله","عبدالرحمن","عبد الرحمن","عبدالعزيز","عبد العزيز","عبدال","عبد","محمود","ابراهيم","إبراهيم","عمر","سامي","سعيد","نادر","ماجد"]);
...
async function renderAll(list,field,query){
  ...
  getAISummary(query,list.slice(0,20)).then(result=>{
    if(result && result.answer){
      ...
      answerWithLinks = answerWithLinks
        .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')  // **bold** to <strong>
        .replace(/\*(.*?)\*/g, '<em>$1</em>')              // *italic* to <em>
        .replace(/~~(.*?)~~/g, '<del>$1</del>')            // ~~strikethrough~~ to <del>
        .replace(/\n/g, '<br>');                            // newlines to <br>
      ...
      answerWithLinks = answerWithLinks.replace(/\[(\d+)\]/g, (match, num) => {
        ...
        if(index >= 0 && index < list.length){
          ...
          const title = (book.raw.title || 'Source ' + num).substring(0, 100);
          if(bookUrl && bookUrl !== '#'){
            return `<a href="${bookUrl}" target="_blank" style="color:#BF9849;font-weight:700;text-decoration:none;border-bottom:2px solid #BF9849;padding:0 2px;background:rgba(191,152,73,0.1);border-radius:3px" title="${title}" onclick="event.stopPropagation()">[${num}]</a>`;
          } else {
            return `<span style="color:#BF9849;font-weight:700;background:rgba(191,152,73,0.1);padding:0 2px;border-radius:3px" title="${title}">[${num}]</span>`;
          }
        }
        return match;
      });
